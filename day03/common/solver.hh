...
template <std::size_t N> auto make_vector_of_bitsets(const std::vector<unsigned>& input) {
    ...
    std::transform(std::begin(input), std::end(input), std::begin(result), [](const auto bits) {
...
auto count_zeros_and_ones(const std::vector<std::bitset<N>>& data, std::size_t n) {
    const auto ones = std::count_if(std::begin(data), std::end(data), [n](const auto& bits) {
        return bits.test(n);
    });
...
template <std::size_t N>
void filter_out_based_on_bit_at_pos(std::vector<std::bitset<N>>& data,
                                    bool bit_is_set,
                                    std::size_t n) {
    const auto it
